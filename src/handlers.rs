...
pub async fn handle_ws_client(websocket: warp::ws::WebSocket) {
    ...
    let config_path = dirs::config_dir().unwrap().into_os_string().into_string().unwrap();
    ...
    let counter = Arc::clone(&shared_sender);
    thread::spawn(move || block_on(handle_provisoning(provisioning_link_rx, counter)));
    ...
    while let Some(body) = receiver.next().await {
        ...
        handle_websocket_message(message, counter, &m).await;
    }
}

async fn handle_provisoning(provisioning_link_rx:Receiver<Url>, sender:Arc<futures::lock::Mutex<SplitSink<WebSocket, warp::ws::Message>>>){
